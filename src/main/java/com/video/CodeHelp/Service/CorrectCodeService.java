...
public class CorrectCodeService {
  ...
  public Long saveCorrectCode(CorrectCodePojo correctCodePojo){
    //lets first try to compile it
    try {
      List<TestCase> defaultTestCase = testCaseService.getTestCases(correctCodePojo.getQNo(),correctCodePojo.getLanguage(), TestCaseType.DEFAULT_TESTCASE);
      compilerFactory.getCompiler(correctCodePojo.getLanguage()).compileCode(CommonUtils.getCodeCompilingRequest(correctCodePojo.getCode(),correctCodePojo.getLanguage(),defaultTestCase,correctCodePojo.getQNo()));
      return dao.saveCorrectCode(correctCodePojo.getCode(), correctCodePojo.getLanguage(), correctCodePojo.getQNo());
  ...
  public CorrectCodePojo getCorrectCode(CorrectCodePojo correctCodePojo) {
    CorrectCodePojo correctCodeFromCache = (CorrectCodePojo)cachingService.getFromCache(CachingUtils.getCacheKeyForCorrectCode(correctCodePojo.getQNo(),correctCodePojo.getLanguage()), CacheTypeEnums.TWO_HUNDERED_CACHE);
    if(correctCodeFromCache == null){
      //reviving it from the database
      CorrectCodePojo correctCode = dao.getCorrectCodeByQNoAndLanguage(correctCodePojo.getQNo(), correctCodePojo.getLanguage());
      if(correctCode!= null){
        cachingService.populateInCache(CachingUtils.getCacheKeyForCorrectCode(correctCode.getQNo(),correctCodePojo.getLanguage()), correctCode, CacheTypeEnums.TWO_HUNDERED_CACHE);
      }
      return correctCode;
    }
    return null;
  ...
  public void updateCorrectCode(CorrectCodePojo correctCodePojo) {
    try {
      compilerFactory.getCompiler(correctCodePojo.getLanguage()).runSimpleCode(correctCodePojo.getCode());
      dao.updateCorrectCode(correctCodePojo.getCode(), correctCodePojo.getQNo(), correctCodePojo.getLanguage());
    } catch (CodeHelpException e) {
      log.error("Error while compiling code for request:{}",correctCodePojo,e);
